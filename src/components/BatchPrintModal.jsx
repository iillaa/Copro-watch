...
import { FaPrint, FaTimes, FaFileAlt, FaListUl, FaUserMd, FaClock, FaCalendarDay } from 'react-icons/fa';
      ...
      <div className="card" style={{ width: '100%', maxWidth: '450px', padding: '1.5rem' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            {count > 1 && (
              <label style={radioStyle}>
                <input type="radio" name="doctype" value="list_manager" checked={docType === 'list_manager'} onChange={(e) => setDocType(e.target.value)} />
            ...
            <label style={radioStyle}>
              <input type="radio" name="doctype" value="convocation" checked={docType === 'convocation'} onChange={(e) => setDocType(e.target.value)} />
              <FaFileAlt style={{color: '#4F46E5'}}/> 
              <span>{count > 1 ? 'Convocations Individuelles' : 'Convocation Médicale'}</span>
            </label>

            <label style={radioStyle}>
              <input type="radio" name="doctype" value="copro" checked={docType === 'copro'} onChange={(e) => setDocType(e.target.value)} />
              <FaFileAlt style={{color: '#059669'}}/> 
              <span>Demande Coproparasitologie</span>
            </label>

            <label style={radioStyle}>
              <input type="radio" name="doctype" value="aptitude" checked={docType === 'aptitude'} onChange={(e) => setDocType(e.target.value)} />
          ...
          {(docType === 'convocation' || docType === 'list_manager') && (
            <div style={{ background: '#f0f9ff', padding: '1rem', borderRadius: '8px', border: '1px dashed #0ea5e9' }}>
              <h4 style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: '#0369a1', display: 'flex', alignItems: 'center', gap: '5px' }}>
                <FaCalendarDay /> Détails du Rendez-vous
              </h4>
              <div style={{ display: 'flex', gap: '10px' }}>
                <div style={{ flex: 1 }}>
                  <label className="label" style={{fontSize: '0.8rem'}}>Date Prévue</label>
                  <input type="date" className="input" required value={consultDate} onChange={(e) => setConsultDate(e.target.value)} />
                </div>
                <div style={{ flex: 1 }}>
                  <label className="label" style={{fontSize: '0.8rem'}}>Heure</label>
                  <div style={{ position: 'relative' }}>
                    <FaClock style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: '#aaa' }} />
                    <input type="time" className="input" style={{ paddingLeft: '30px' }} required value={consultTime} onChange={(e) => setConsultTime(e.target.value)} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="btn btn-primary">Générer PDF</button>
          </div>
        </form>
      </div>
    </div>
