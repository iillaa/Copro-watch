...
import { FaArrowLeft, FaFileMedical, FaTrash, FaArchive, FaBoxOpen, FaStethoscope } from 'react-icons/fa';
...
export default function MobileWorkerDetail({ workerId, onBack }) {
  ...
  const handleToggleArchive = async () => {
    ...
    if (window.confirm(`Voulez-vous vraiment ${newStatus ? 'archiver' : 'réactiver'} ce travailleur ?`)) {
  ...
  return (
    <div className="mobile-layout">
      ...
      <div style={{ padding: '0.5rem', borderBottom: '1px solid #eee', background: 'white', position:'sticky', top:0, zIndex:10, display:'flex', alignItems:'center', gap:'1rem' }}>
        <button onClick={onBack} className="btn btn-sm btn-outline" style={{border:'none', padding:'0.5rem'}}>
            <FaArrowLeft size={18} />
        </button>
      ...
      {/* Profile Card */}
      <div className="mobile-card" style={{ marginTop: '1rem', borderTop: `4px solid ${worker.archived ? '#999' : 'var(--primary)'}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '0.5rem' }}>
            <button className="btn btn-primary" onClick={() => { setSelectedExam(null); setShowExamForm(true); }}>
                <FaFileMedical /> Exam
            </button>
            <button className="btn btn-outline" onClick={handleToggleArchive} style={{ padding:'0 1rem' }}>
                {worker.archived ? <FaBoxOpen /> : <FaArchive />}
            </button>
            <button className="btn btn-outline" onClick={handleDeleteWorker} style={{ color:'var(--danger)', borderColor:'var(--danger)', padding:'0 1rem' }}>
                <FaTrash />
            </button>
        </div>
      </div>
      ...
      <h3 style={{ padding: '0 0.5rem', fontSize: '1.1rem', marginTop: '1.5rem' }}>Historique Médical</h3>
      
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {exams.map((e) => (
          <div key={e.id} className="mobile-card" style={{ marginBottom: 0 }} onClick={() => { setSelectedExam(e); setShowExamForm(true); }}>
      ...
      {showExamForm && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'white' }}>
            ...
            <button 
                ...
                style={{ position:'fixed', top:'1rem', right:'1rem', zIndex:2001, background:'black', color:'white', border:'none', borderRadius:'50%', width:'30px', height:'30px' }}
            >
                ✕
            </button>
        </div>
