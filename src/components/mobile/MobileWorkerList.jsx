...
export default function MobileWorkerList({ onNavigateWorker }) {
  ...
  const loadData = async () => { setWorkers(await db.getWorkers()); };
  useEffect(() => { loadData(); }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (deferredSearch) {
      ...
      res = res.filter(w => w.full_name.toLowerCase().includes(lower) || w.national_id.includes(lower));
    }
    return res;
  ...
  return (
    <div className="mobile-layout">
      {/* Search Header */}
      <div style={{ position: 'sticky', top: 0, background: 'var(--bg-app)', zIndex: 10, padding: '0.5rem 1rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
            <div className="input" style={{ display: 'flex', alignItems: 'center', padding: '0.5rem', borderRadius: '50px', flex: 1 }}>
            ...
            <button className="btn btn-primary" style={{ borderRadius: '50%', width: '45px', padding: 0 }} onClick={() => { setEditingWorker(null); setShowForm(true); }}>
                <FaPlus />
            </button>
        </div>
      </div>
        ...
        {filtered.map(w => (
            <div key={w.id} className="mobile-card" onClick={() => onNavigateWorker(w.id)}>
                ...
                <div className="mobile-card-actions">
                    <button className="btn btn-sm btn-outline" onClick={(e) => { e.stopPropagation(); setEditingWorker(w); setShowForm(true); }}>
                        <FaUserEdit /> Éditer
                    </button>
                </div>
            </div>
