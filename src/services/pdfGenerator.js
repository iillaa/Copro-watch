...
function generateGroupedList(doc, workers, options) {
  ...
  deptNames.forEach((dept, i) => {
    ...
    doc.setFontSize(8);
    doc.text("Le Chef de Service est prié de faire signer les employés convoqués.", 105, 285, { align: "center" });
  });
}
...
function drawSmartAptitude(doc, worker, options) {
  ...
  const locationText = `${worker.deptName || ''} ${worker.workplaceName ? ' / ' + worker.workplaceName : ''}`;
...
function drawFooter(doc) {
  ...
  doc.text("Généré par Copro Watch - Usage interne uniquement", 105, pageHeight - 10, { align: "center" });
